...
def step5_summary(conf_dict,logfile,filelist):
    ...
    # rwlog('cp %s %s'%(conf_dict['Step1_preprocess']['profilebw']  , summarydir),logfile)    
    # rwlog('cp %s %s'%(conf_dict['Step1_preprocess']['mapped_bed']  , summarydir),logfile)
    # if os.path.exists(conf_dict['Step1_preprocess']['mapped_bb']):
    #     rwlog('cp %s %s'%(conf_dict['Step1_preprocess']['mapped_bb']  , summarydir),logfile)    
    # rwlog('cp %s %s'%(conf_dict['Step2_QC']['ave_tssprofile']  , summarydir),logfile)
    # if int(conf_dict['Step2_QC']['plotcustom']) == 1:
    #     conf_dict['Step2_QC']['custom_profile'] = qcdir + conf_dict['General']['outname'] + '_CustomRegion_profile.txt'
    #     rwlog('cp %s %s'%(conf_dict['Step2_QC']['custom_profile']  , summarydir),logfile)
    # rwlog('cp %s %s'%(conf_dict['Step3_nucarray']['arrayselect']  , summarydir),logfile)
    # rwlog('cp %s %s'%(conf_dict['Step3_nucarray']['gene_array_anno']  , summarydir),logfile)
    # rwlog('cp %s %s'%(conf_dict['Step3_nucarray']['profilebw_onarray']  , summarydir),logfile)
    # rwlog('cp %s %s'%(conf_dict['Step3_nucarray']['positionbw_center']  , summarydir),logfile)
    ...
    # inf = open(summarydir + conf_dict['General']['outname'] + '_Parameters.txt','w')
    # for each in conf_dict.keys():
    #     inf.write("#%s" % each+"\n")
    #     for each_item in conf_dict[each].keys():
    #         inf.write("\t#%s:%s\n" % (each_item,conf_dict[each][each_item]))
    # inf.close()
...
\\newpage
\\newpage
\subsection{Nucleosome profile on potential funcitonal regions}
\\begin{quotation}
...
\end{quotation}
\\begin{figure}[h]
        \caption{Nucleosome profile on potential funcitonal regions} \label{fig:profileunion}
        \setlength{\\abovecaptionskip}{0pt}
        \setlength{\\belowcaptionskip}{10pt}
        \centering
        {\includegraphics[width=0.8\\textwidth]{%s}}
...
"""%(
    ...
    if conf_dict['clean']:
        ...
        rwlog("rm %s "%(conf_dict['General']['outputdirectory'] + 'preprocess/rawbed_sortbychrm_tmp.bed'),logfile)
        rwlog("rm %s "%(conf_dict['General']['outputdirectory'] + 'preprocess/'+conf_dict['General']['outputdirectory']+'.bed.tmp'),logfile)
