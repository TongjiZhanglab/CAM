...
def step1_preprocess(conf_dict,logfile,filelist):
        ...
        if check_filelist(filelist,conf_dict['Step1_preprocess']['sam'])==0 or not os.path.isfile(conf_dict['Step1_preprocess']['sam']):
            ...
            if conf_dict['General']['seqtype'] == 'PE':
                ...
                if ReadLengthB[1] == 'difflen':
                    wlog('WARNING: reads length in input fastq (part2) %s is not consistant, CAM exit'%(conf_dict['General']['inputb']),logfile)
    ...
    # PE to PE, random the start or end of the 147bp center fragment. storage only 1bp
    if conf_dict['General']['format'] == 'fastq' or conf_dict['General']['format'] == 'sam':
        # check the progress for samtobed
        # count the mapped reads and storage fragments distribution for PE data
        # for SE data, only count the mapped reads, and transform sam to bed
    ...
    if conf_dict['General']['seqtype']=='PE':
        # storage fragment distribution on preprocess/freglen_tmp.txt temporarily
